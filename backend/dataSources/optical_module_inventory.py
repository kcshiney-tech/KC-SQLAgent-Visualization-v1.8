...
def get_cluster_info_from_hostname(hostname: str) -> Tuple[str, str]:
    ...
    # 检查是否匹配CORE模式
    core_match = re.match(r'^[A-Z0-9-]*-ROCE-CORE-\d+', hostname)
    if core_match:
        # 提取机房信息，从主机名前面两个字符串拼接在一起
        parts = hostname.split('-')
        if len(parts) >= 2:
            ...
            if parts[0] == parts[1]:
                ...
                idc = parts[0] + parts[1]
            # 查找数字部分
            num_match = re.search(r'-ROCE-CORE-(\d+)', hostname)
            if num_match:
                number = num_match.group(1)
    ...
    # 检查是否匹配SCORE模式
    score_match = re.match(r'^[A-Z0-9-]*-SROCE-CORE-\d+', hostname)
    if score_match:
        # 提取机房信息，从主机名前面两个字符串拼接在一起
        parts = hostname.split('-')
        if len(parts) >= 2:
            ...
            if parts[0] == parts[1]:
                ...
                idc = parts[0] + parts[1]
            # 查找数字部分
            num_match = re.search(r'-SROCE-CORE-(\d+)', hostname)
            if num_match:
                number = num_match.group(1)
